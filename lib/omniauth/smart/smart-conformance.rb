...
class SmartConformance
  ...
  def self.get_conformance_from_server(service_uri)
    ...
    SmartConformance.new(conformance_json)
  end

  def initialize(conformance_json)
    raise SmartConformanceError.new("Expecting json hash, instead go #{conformance_json.class}") unless conformance_json.is_a?(Hash)
  ...
  def parse
    ...
    # todo: read FHIR version
    # ["Conformance"]["FHIR"]["Version"]
    # example: "fhirVersion": "1.0.2"

    raise SmartConformanceError.new("No authorization uri") unless @authorize_url
    raise SmartConformanceError.new("No token uri") unless @token_url
